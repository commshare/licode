...
#include "server/utils/IniFile.h"
...
void cRoot::Start(std::unique_ptr<cSettingsRepositoryInterface> overridesRepo)
{
  ...
  //控制台日志
	...
	auto consoleAttachment = zhangbin::cLogger::GetInstance().AttachListener(std::move(consoleLogListener));
	...
	auto fileAttachment = zhangbin::cLogger::GetInstance().AttachListener(std::move(fileLogListenerRet.second));
	...
	while (!m_ShouldStop)
	{
      ...
      // These are modified by external threads
			while (!m_ShouldStop && !m_bRestart && !m_TerminateEventRaised)
			{
